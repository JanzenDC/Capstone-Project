  ...
  class API{
    ...
    public function httpGet($payload)
    {
        if($payload['select']){
            ...
            WHERE mpo_tbl.mpoID = ?';
            $queryResult = $this->db->rawQuery($sqlQuery, array($selected));
            if($queryResult){
                $mpobasedata = $this->db->where('mpoID',$payload['select'])->get("mpo_base");
                $response = [
                    'status' => 'success',
                    'message' => 'Data has been fetch succesfully',
                    'queryResult' => $queryResult,
                    'mpoBaseFetch' => $mpobasedata,
